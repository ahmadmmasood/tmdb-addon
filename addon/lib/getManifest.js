...
function setOrderLanguage(language, languagesArray) {
  ...
  return [
    preferred.name,
    ...list
      .filter((l) => l.name !== preferred.name)
      .map((l) => l.name),
  ];
}
...
async function getManifest(config = {}) {
  ...
  try {
    const movieGenres = await getGenreList(
      language,
      "movie",
      config
    );

    const seriesGenres = await getGenreList(
      language,
      "series",
      config
    );
  ...
  try {
    const languagesArray =
      await getLanguages(config);

    filterLanguages = setOrderLanguage(
      language,
      languagesArray
    );
  } catch (e) {
